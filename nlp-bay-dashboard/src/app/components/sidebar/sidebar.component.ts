...
export class SidebarComponent implements OnInit, AfterViewInit {
  ...
  getBaseUrlAndThenProjectPlan() {
    ...
    if (dshbrdBaseUrl.includes('localhost')) {
  ...
  getOSCODE() {
    this.public_Key = this.appConfigService.getConfig().t2y12PruGU9wUtEGzBJfolMIgK;
    this.logger.log('[SIDEBAR] AppConfigService public_Key:', this.public_Key);
  
    // Map of keys to visibility properties
    const visibilityMap = {
      PAY: "isVisiblePAY",
      ANA: "isVisibleANA",
      ACT: "isVisibleACT",
    ...
    // Initialize all visibility properties to false
    Object.keys(visibilityMap).forEach((key) => {
      this[visibilityMap[key]] = false;
    });
  
    // Extract keys from the public key
    const keys = this.public_Key.split(":");
  
    // Update visibility based on the public key
    keys.forEach((segment) => {
      const [key, value] = segment.split("-");
      if (visibilityMap[key]) {
        this[visibilityMap[key]] = value === "T";
      }
    });
  }
