...
class InstagramClient {
   ...
   */
  constructor(config) {
    if (!config.INSTA_GRAPH_URL) {
      throw new Error("INSTA_GRAPH_URL is mandatory.");
    }
    if (!config.INSTA_APP_ID) {
      throw new Error("APP_ID is mandatory.");
    }
    if (!config.INSTA_APP_SECRET) {
      throw new Error("config.APP_SECRET is mandatory");
    }
    if (!config.BASE_URL) {
      throw new Error("config.BASE_URL is mandatory");
    }
    this.insta_graph_url = config.INSTA_GRAPH_URL;
    this.app_id = config.INSTA_APP_ID;
    this.app_secret = config.INSTA_APP_SECRET;
    this.base_url = config.BASE_URL;
  }
  ...
  async getAccessTokenFromCode(code, callback) {
    ...
    const URL = this.insta_graph_url + 'oauth/access_token';
    //const URL = "https://graph.facebook.com/v9.0/oauth/access_token";
    const HTTPREQUEST = {
      url: URL,
      headers: {
  ...
  async messageEventSubscription(app_id, access_token) {
    https://graph.instagram.com/v21.0/1755847768034402/subscribed_apps
    return await axios({
  ...
  getInstagramBusinessAccount = async (accessToken) => {
    try {
        const response = await axios.get(`${this.insta_graph_url}me?fields=instagram_business_account&access_token=${accessToken}`);
